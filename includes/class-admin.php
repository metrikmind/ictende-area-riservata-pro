...
class AreaRiservata_Admin {
    ...
    public function init() {
        ...
        add_action('wp_ajax_ar_clear_all_logs', array($this, 'ajax_clear_all_logs')); // NUOVO
    }
    ...
    public function enqueue_scripts($hook) {
        ...
        wp_localize_script('area-riservata-admin', 'areaRiservataAdmin', array(
            ...
            'confirm_clear_logs' => __('Sei sicuro di voler eliminare tutte le attività recenti? Questa azione non può essere annullata.')
        ));
    }
    ...
    /**
     * Tab Panoramica - SENZA AZIONI RAPIDE
     */
    private function render_overview_tab() {
        ...
        <!-- ATTIVITA' RECENTI CON PULSANTE ELIMINA TUTTO -->
        <div class="recent-activity">
            <div class="activity-header">
                <h3>Attività Recenti</h3>
                <?php if ($recent_logs): ?>
                    <button class="clear-logs-btn" id="clear-all-logs">
                        <span class="dashicons dashicons-trash"></span> Elimina Tutto
                    </button>
                <?php endif; ?>
            </div>
    ...
    /**
     * Tab Utenti - PULSANTI AZIONI MIGLIORATI
     */
                                    ...
                                    <td>
                                        <div class="row-actions">
                                            <?php if ($user->status == 'pending'): ?>
                                                <button class="button approve-user" 
                                                        data-user-id="<?php echo esc_attr($user->id); ?>"
                                                        title="Approva utente">
                                                    ✓ Approva
                                                </button>
                                                <button class="button reject-user" 
                                                        data-user-id="<?php echo esc_attr($user->id); ?>"
                                            ...
                                            <?php else: ?>
                                                <button class="button toggle-status" 
                                                        ...
                                                        title="<?php echo $user->status == 'approved' ? 'Disabilita' : 'Abilita'; ?> utente">
                                                    <?php echo $user->status == 'approved' ? '⏸ Disabilita' : '▶ Abilita'; ?>
                                                </button>
                                            <?php endif; ?>
                                            
                                            <button class="button view-user" 
                                                    data-user-id="<?php echo esc_attr($user->id); ?>"
    ...
    /**
     * Tab Documenti - CON ELIMINAZIONE CORRETTA
     */
                        ...
                        <div class="document-card" data-category="<?php echo esc_attr($doc->categoria); ?>" 
                            ...
                            <div class="doc-header">
                                ...
                                <div class="doc-actions">
                                    <button class="button delete-doc" 
                                            data-doc-id="<?php echo esc_attr($doc->id); ?>"
    ...
    public function ajax_delete_document() {
        ...
        // Debug
        error_log('AREA RISERVATA: Tentativo eliminazione documento ID: ' . $doc_id);
        
        $document = $this->db->get_document($doc_id);
        
        if (!$document) {
            error_log('AREA RISERVATA: Documento non trovato nel database ID: ' . $doc_id);
            wp_send_json_error(array('message' => 'Documento non trovato nel database'));
            return;
        }
        
        error_log('AREA RISERVATA: Documento trovato: ' . $document->nome_file . ' Path: ' . $document->path_file);
        
        // Elimina file fisico
        $upload_dir = wp_upload_dir();
        $file_path = $upload_dir['basedir'] . '/area-riservata/' . $document->path_file;
        
        error_log('AREA RISERVATA: Percorso file completo: ' . $file_path);
        
        if (file_exists($file_path)) {
            if (unlink($file_path)) {
                error_log('AREA RISERVATA: File fisico eliminato con successo');
            } else {
                error_log('AREA RISERVATA: Errore eliminazione file fisico');
            }
        } else {
            error_log('AREA RISERVATA: File fisico non trovato, procedo con eliminazione DB');
        }
        ...
        if ($result !== false) {
            error_log('AREA RISERVATA: Record database eliminato con successo');
            ...
            error_log('AREA RISERVATA: Errore eliminazione record database');
            wp_send_json_error(array('message' => 'Errore nell\'eliminazione del documento dal database'));
        }
    }
    ...
    public function ajax_clear_all_logs() {
        ...
        if ($result !== false) {
            // Inserisci un log per questa azione
            $wpdb->insert($logs_table, array(
                'user_id' => 0,
                'azione' => 'Log eliminati',
                'dettagli' => 'Tutti i log delle attività sono stati eliminati dall\'amministratore',
                'ip_address' => $_SERVER['REMOTE_ADDR'] ?? 'Unknown',
                'created_at' => current_time('mysql')
            ));
            
            wp_send_json_success(array(
                'message' => 'Tutti i log delle attività sono stati eliminati con successo'
            ));
        } else {
            wp_send_json_error(array('message' => 'Errore durante l\'eliminazione dei log'));
        }
    }
