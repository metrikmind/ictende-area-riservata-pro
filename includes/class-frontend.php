...
class AreaRiservata_Frontend {
    ...
    public function init() {
        ...
        // NASCONDE ADMIN BAR NELLA DASHBOARD
        add_action('wp_head', array($this, 'hide_admin_bar_dashboard'));
        add_filter('show_admin_bar', array($this, 'hide_admin_bar_filter'));
    }
    
    /**
     * Nasconde l'admin bar nella dashboard
     */
    public function hide_admin_bar_dashboard() {
        global $post;
        
        // Nasconde admin bar se siamo in una pagina con shortcode dashboard
    ...
    /**
     * Filtro per nascondere admin bar
     */
    public function hide_admin_bar_filter($show) {
        global $post;
        
        // Nasconde solo se siamo in una pagina dashboard dell'area riservata
        if ($post && has_shortcode($post->post_content, 'area_riservata_dashboard')) {
            return false;
        }
    ...
    public function add_body_classes($classes) {
        ...
        // Aggiunge classe per pagine con dashboard
        global $post;
        if ($post && has_shortcode($post->post_content, 'area_riservata_dashboard')) {
            $classes[] = 'area-riservata-dashboard';
        }
    ...
    /**
     * Shortcode Dashboard - Con logout che riporta al login - NASCONDI ADMIN BAR
     */
