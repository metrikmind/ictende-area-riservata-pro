/* Area Riservata Admin JavaScript - Versione Aggiornata */
...
jQuery(document).ready(function($) {
    ...
    const config = {
        ...
        confirmClearLogs: areaRiservataAdmin.confirm_clear_logs || 'Sei sicuro di voler eliminare tutte le attività recenti?'
    };
    ...
    const usersHandler = {
        init: function() {
            $('#add-user-btn').on('click', this.showAddUserModal.bind(this));
    ...
    const documentsHandler = {
        init: function() {
            $('#upload-doc-btn').on('click', this.showUploadModal.bind(this));
        ...
        deleteDocument: function(e) {
            ...
            console.log('Tentativo eliminazione documento ID:', docId);
            
            if (!docId) {
                console.error('ID documento non trovato');
                alert('Errore: ID documento non trovato');
                return;
            }
            ...
            if (confirm(config.confirmDelete)) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            // Trova il documento card corretto usando data-doc-id
                            const $card = $(`.document-card[data-doc-id="${docId}"]`);
                            console.log('Card trovata:', $card.length);
                            $card.fadeOut(() => {
                                $card.remove();
                            });
                        } else {
                            alert(response.data.message || 'Errore durante l\'eliminazione del documento');
                        }
                    },
    ...
    const logsHandler = {
        init: function() {
            ...
            $('#clear-all-logs').on('click', this.clearAllLogs.bind(this)); // NUOVO
        },
        ...
        clearAllLogs: function() {
            if (confirm(config.confirmClearLogs)) {
                $.ajax({
                    url: config.ajaxUrl,
                    type: 'POST',
                    data: {
                        action: 'ar_clear_all_logs',
                        nonce: config.nonce
                    },
                    success: function(response) {
                        if (response.success) {
                            // Rimuovi tutti gli elementi dalla lista attività
                            $('#activity-list').fadeOut(() => {
                                $('#activity-list').html('<p>Nessuna attività registrata.</p>').fadeIn();
                            });
                            
                            // Nascondi il pulsante elimina tutto
                            $('.clear-logs-btn').fadeOut();
                            
                            alert(response.data.message);
                        } else {
                            alert(response.data.message || 'Errore durante l\'eliminazione dei log');
                        }
                    },
                    error: function() {
                        alert('Errore di connessione');
